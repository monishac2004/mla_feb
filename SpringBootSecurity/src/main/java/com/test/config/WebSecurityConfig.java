...
public class WebSecurityConfig {
	...
	protected UserDetailsService userDetailsService(){
		...
		UserDetails hr = User.builder().username("HR").password(passwordEncoder().encode("MON")).roles("HR").build();
		
		UserDetails emp = User.builder().username("emp").password(passwordEncoder().encode("min")).roles("HR", "EMPLOYEE").build();
	...
	protected SecurityFilterChain filterChain(HttpSecurity http){
		return http.csrf(AbstractHttpConfigurer :: disable).authorizeHttpRequests(
				...
				.permitAll())
				.logout(config -> 
				config.logoutUrl("/logout").logoutSuccessUrl("/login")).build();
